...
long	 sitronix_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case IOCTL_SMT_ENABLE_IRQ:
			...
			enable_irq(gpio_to_irq(sitronix_ts_gpts->client->irq));
			break;
		case IOCTL_SMT_DISABLE_IRQ:
			UpgradeMsg("IOCTL_SMT_DISABLE_IRQ\n");
			disable_irq_nosync(gpio_to_irq(sitronix_ts_gpts->client->irq));
			break;
		default:
			retval = -ENOTTY;
	}
...
static int	sitronix_ts_get_status(struct sitronix_ts_data *ts, uint8_t *status)
{
	uint8_t	buffer[2];
	int ret=0;

	//printk("====>sitronix_get_status\n");
	buffer[0] = STATUS_REG;
	ret = i2c_master_send(ts->client, buffer, 1);
	if (ret < 0){
		//printk("get status command error (%d)\n", ret);
		return ret;
	}

	//printk("sitronix_ts_get_status. calling i2c_master_recv\n");
	ret = i2c_master_recv(ts->client, &buffer[1], 1);
	if (ret < 0){
		//printk("read device control status error (%d)\n", ret);
		*status = -1;
	}else{
	...
	//printk("<====sitronix_get_status value=0x%x\n", *status);
	return ret;
}
...
static int sitronix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	set_bit(KEY_HOME, ts->keyevent_input->keybit);
	set_bit(KEY_MENU, ts->keyevent_input->keybit);
	set_bit(KEY_BACK, ts->keyevent_input->keybit);
	set_bit(KEY_SEARCH, ts->keyevent_input->keybit);
	input_set_abs_params(ts->keyevent_input, ABS_X, 0, 0, 0, 0);
#endif // defined(SITRONIX_SENSOR_KEY) || defined (SITRONIX_TOUCH_KEY)
	...
	if (client->irq){
#ifdef SITRONIX_LEVEL_TRIGGERED
		ret = request_irq(gpio_to_irq(client->irq), sitronix_ts_irq_handler, IRQF_TRIGGER_LOW | IRQF_DISABLED, client->name, ts);
#else
		ret = request_irq(gpio_to_irq(client->irq), sitronix_ts_irq_handler, IRQF_TRIGGER_FALLING | IRQF_DISABLED, client->name, ts);
#endif // SITRONIX_LEVEL_TRIGGERED
		if (ret == 0)
			ts->use_irq = 1;
		else
