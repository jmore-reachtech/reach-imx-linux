 ...
 * Copyright 2012 Reach Technology
 *
...
#define PCF8523_CTL3_BA_EN_MASK	0xE0
#define PCF8523_REG_CLKO_MASK	0x38
...
static int pcf8523_setup(struct pcf8523 *pcf8523)
{
	struct i2c_client	*client = pcf8523->client;
	int ret,val;
	
	/* read clock out reg */
	if((ret = i2c_smbus_read_byte_data(client,PCF8523_REG_CLKO)) < 0) {
		printk("%s CLKO read error 0x%X \n",__func__ ,ret);
		return -EIO;
	} else {
		printk("%s CLKO read = 0x%X \n",__func__ ,ret);
	}
	
	/* disable clock out */
	val = (ret | PCF8523_REG_CLKO_MASK);
	printk("%s setting CLKO = 0x%X \n",__func__ ,val);
	if((ret = i2c_smbus_write_byte_data(client,PCF8523_REG_CLKO,val)) < 0) {
		printk("%s CLKO write error 0x%X \n",__func__ ,ret);
		return -EIO;
	}
